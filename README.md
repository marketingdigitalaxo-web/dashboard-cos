...
- Tabla por campaña con el embudo completo: View → Click (CTR) → Add to
  cart (% que agrega desde quienes hicieron click) → Purchase (% que
  compra desde quienes agregaron al carro) → conversión total (Purchase
  / View) → ingresos. Esto es lo que te deja ver, por ejemplo, si una
  campaña tiene buen CTR pero se cae en "agregar al carro" — el problema
  no es que no llame la atención, es lo que pasa después del click.
